...
class AttendanceService:
    ...
    @staticmethod
    def get_attendance_lecture_time(attendance_time: datetime.datetime, lab_lectures: list[LectureTimes]) -> Optional[LectureTimes]:
    ...
    async def get_attendances_by_lab_and_date(lab_id: int, date: datetime.date) -> AttendancesByLabAndDateGetDTO:
        ...
        attendance_get_dtos = [AttendanceGetDTO(card_id=attendance.card_id, attendance_time=attendance.date) for attendance in attendances]
        ...
        for attendance in attendance_get_dtos:
            lecture_time = AttendanceService.get_attendance_lecture_time(attendance.attendance_time, sorted_lab_lectures)
            if lecture_time:
        ...
        return AttendancesByLabAndDateGetDTO(lab_id=lab_id, date=date, lectures_attendances=lecture_attendances, out_of_lectures_attendances=out_of_lecture_attendances)
