...
async def insert_labs_fake_data():
    ...
    for lab in labs:
        fake_labs.append(LabsModel(name=lab["name"], number=lab["number"],
                                   last_updated_date=DatetimeService.convert_date_string_to_datetime(
                                       lab["last_updated_date"]), lecture_times=lab["lecture_times"]))
...
async def insert_attendances_fake_data():
    ...
    for record in attendance_records:
        fake_attendances.append(AttendancesModel(card_id=record["card_id"],
                                                 date=DatetimeService.convert_date_string_to_datetime(record["date"]),
